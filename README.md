...
TimeLens 是一个本地优先的“个人照片智能浏览器 / 记忆系统”项目。当前仓库已经重构为前后端分离架构：FastAPI 负责 API、扫描与媒体访问，独立 React 前端负责浏览体验和后续复杂交互。
...
app/
  ...
  templates/          旧版 SSR 模板（迁移期保留，不再作为主入口）
  static/             旧版静态资源（迁移期保留）
...
后端启动后访问 [http://127.0.0.1:8000](http://127.0.0.1:8000)，默认返回 API 模式说明。
...
默认会请求 `http://127.0.0.1:8000/api`，可以在 `frontend/.env` 中通过 `VITE_API_BASE_URL` 修改。
...
- 前后端分离：后端专注索引、分析、媒体与 API，前端专注浏览体验与未来复杂交互。
